...
from automation.media import control_media, play_media
from integrations.open_app import open_app
from integrations.reminders import add_reminder, get_reminders
from integrations.screen_vision import analyze_screen
from integrations.service_status import service_status
from integrations.tcp_status import tcp_status
from automation.shell import shell_run
from integrations.system_info import sys_info
from automation.system_power import system_sleep
from integrations.weather import get_weather_summary
from automation.whatsapp import (
    call_whatsapp_contact,
    save_whatsapp_contact,
    send_whatsapp_message,
)
from integrations.youtube_stats import get_youtube_channel_report
from integrations.home_assistant import home_control
...
from security.owner import has_owner_pin, verify_owner_pin
...
def load_system_prompt() -> str:
    try:
        ...
        return (
            "Sen JARVIS'sin. Windows Ã¼zerinde Ã§alÄ±ÅŸan kiÅŸisel AI asistanÄ±sÄ±n. "
...
class JarvisLive:
    ...
    def _on_text_command(self, text: str):
        ...
        if self._handle_owner_command(text):
            return
        service_query = self._service_status_query(text)
        if service_query:
            self.ui.write_log(f"JARVIS: {service_status(service_query)}")
            return
        tcp_query = self._tcp_status_query(text)
        if tcp_query:
            self.ui.write_log(f"JARVIS: {tcp_status(tcp_query)}")
            return

        if not self._loop or not self.session or self._loop.is_closed():
            self.ui.write_log("ERR: JARVIS baÄŸlantÄ±sÄ± hazÄ±r deÄŸil.")
            return
    ...
    def _log_async_send_error(self, future):
        try:
            ...
            self.ui.write_log(f"ERR: Mesaj gÃ¶nderilemedi: {e}")

    @staticmethod
    def _service_status_query(text: str) -> str | None:
        q = (
            str(text or "")
            .strip()
            .lower()
        ...
        if "home server" in q and "durum" in q:
            return "home_server"
        if "cevrimdisi" in q or "offline" in q:
            return "offline"
        if "baglanti olay" in q or "son olay" in q or "event" in q:
            return "events"
        if "saglik rapor" in q or "system health" in q:
            return "health"
        if "bagli servis" in q or "connected services" in q or "servisleri goster" in q:
            return "connected"
        return None

    @staticmethod
    def _tcp_status_query(text: str) -> str | None:
        q = (
            str(text or "")
            .strip()
            .lower()
        ...
        if "tcp" not in q and "baglanti" not in q:
            return None
        if "home server" in q:
            return "home_server"
        if "supheli" in q or "suspicious" in q or "risk" in q:
            return "suspicious"
        if "olay" in q or "event" in q or "son" in q:
            return "events"
        if "istatistik" in q or "stats" in q or "say" in q:
            return "stats"
        if "tcp" in q:
            return "connections"
        return None
    ...
    def _handle_owner_command(self, text: str) -> bool:
        ...
        if verify_owner_pin(pin):
            ...
            self.ui.write_log("JARVIS: Sahip doÄŸrulandÄ±. Hassas araÃ§lar aÃ§Ä±ldÄ±.")
            self._ui_call("write_debug", "Owner session unlocked", level="SECURITY")
        else:
            self._owner_authorized = False
            self.ui.write_log("JARVIS: Yetki kodu hatalÄ± veya sahip PIN ayarlÄ± deÄŸil.")
    ...
    def _owner_guard_result(self, tool_name: str) -> Optional[str]:
        ...
        if not has_owner_pin():
            return (
                "GÃ¼venlik kilidi aktif ama sahip PIN ayarlÄ± deÄŸil. "
                "Terminalde proje klasÃ¶rÃ¼nde ÅŸu komutla PIN ayarla: "
    ...
    def _should_play_success_sfx(tool_name: str, args: dict, result) -> bool:
        ...
        if tool_name == "send_whatsapp_message":
            ...
            return bool(args.get("send_now", False)) and (
                "gÃ¶nderildi" in text or "gonderildi" in text
            )
        return False
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            if name == "owner_unlock":
                ...
                if verify_owner_pin(pin):
                    self._owner_authorized = True
                    result = "Sahip doÄŸrulandÄ±. Hassas araÃ§lar aÃ§Ä±ldÄ±."
                else:
                    self._owner_authorized = False
                    result = "Yetki kodu hatalÄ± veya sahip PIN ayarlÄ± deÄŸil."
                ...
                result = result or "Windows uyku moduna geÃ§iyor."
                ...
                result = result or f"{args.get('app_name')} aÃ§Ä±ldÄ±."
                ...
                result = result or "Bilgi alÄ±ndÄ±."

            elif name == "service_status":
                result = await loop.run_in_executor(
                    None,
                    lambda: service_status(args.get("query", "connected")),
                )
                result = result or "Servis durumu alindi."

            elif name == "tcp_status":
                result = await loop.run_in_executor(
                    None,
                    lambda: tcp_status(args.get("query", "connections")),
                )
                result = result or "TCP durumu alindi."
                ...
                result = result or "Hava durumu bilgisi alÄ±ndÄ±."
                ...
                result = result or "Takvim bilgisi alÄ±ndÄ±."
                ...
                result = result or "Takvim etkinliÄŸi eklendi."
                ...
                result = result or "Takvim etkinliÄŸi silindi."
                ...
                result = result or "AnÄ±msatÄ±cÄ± bilgisi alÄ±ndÄ±."
                ...
                result = result or "AnÄ±msatÄ±cÄ± eklendi."
                ...
                result = result or "Komut Ã§alÄ±ÅŸtÄ±rÄ±ldÄ±."
                ...
                result = result or "Medya oynatma baÅŸlatÄ±ldÄ±."
                ...
                result = result or "Medya komutu gÃ¶nderildi."
                ...
                result = result or "YouTube kanal raporu alÄ±ndÄ±."
                ...
                result = result or "Ekran analizi tamamlandÄ±."
                ...
                result = result or "WhatsApp iÅŸlemi tamamlandÄ±."
                ...
                result = result or "WhatsApp arama iÅŸlemi tamamlandÄ±."
                ...
                result = result or "WhatsApp kiÅŸisi kaydedildi."
    ...
    async def _listen_audio(self, session):
        ...
        try:
            ...
            self.ui.write_log(f"ERR: Mikrofon hatasÄ±: {e}")
            raise
        finally:
            if stream:
    ...
    def _flush_transcripts(
        ...
        if output_noise:
            self.ui.write_log("ERR: JARVIS sesli yanÄ±tÄ±nÄ± Ã§Ã¶zÃ¼mlerken hata oluÅŸtu.")
            if output_noise_samples:
                self._ui_call(
                    "write_debug",
    ...
    async def _play_audio(self, session):
        ...
        try:
            ...
            self.ui.write_log(f"ERR: Ses Ã§Ä±kÄ±ÅŸÄ± hatasÄ±: {e}")
            raise
        finally:
            self.set_speaking(False)
            if stream:
    ...
    async def run(self):
        ...
        while True:
            try:
                self._ui_call("set_state", "THINKING")
                self.ui.write_log("SYS: JARVIS baÄŸlanÄ±yor...")
                ...
                self.session = None
                self.ui.write_log(
                    f"ERR: JARVIS baÄŸlantÄ±sÄ± kesildi veya internete ulaÅŸÄ±lamÄ±yor - {e}"
                )
                self._ui_call("set_state", "ERROR")
